...
trait KataSpec extends FlatSpec with Matchers {
  ...
  implicit class FutureOps[T](f: Future[T]) {
    def await = Await.result(f, Duration.create(2, TimeUnit.SECONDS))
  }
...
class CharacterSpec extends KataSpec {
  ...
  it should "receive damage" in {
    thrud.damage(hp100).await shouldBe thrud900HitPoints
    thrud200HitPoints.damage(hp100).await shouldBe thrud100HitPoints

  }
  behavior of "receiving negative damage"

  it should "ignore -ve damage" in {
    thrud200HitPoints.damage(hpMinus100).await shouldBe thrud200HitPoints
  }

  behavior of "Death"

  it should "die if damage received takes it negative" in {
    thrud100HitPoints.damage(hp900).await shouldBe deadThrud
  }
  ...
  it should "call the logging function" in {
    val (x, msg) = valueAndLog(thrud.damage(hp100).await)
